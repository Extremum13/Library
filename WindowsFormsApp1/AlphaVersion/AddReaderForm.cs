...
namespace Library.AlphaVersion
{
    public partial class AddReaderForm : Form
    {
        ...
        {
            InitializeComponent();
            dateTimePicker.MaxDate = DateTime.UtcNow.AddYears(-14);
        }
        ...
        {
            TextBox thisTextBox = (sender as TextBox);
            if (Validation.Validation.EmailValidation(thisTextBox.Text))
            {
                thisTextBox.BackColor = Color.White;
            }
            else
        ...
        private void emailTextBox_TextChanged(object sender, EventArgs e)
        {
            ...
            if(!Validation.Validation.EmailEnteringValidation(thisTextBox.Text))
            {                
                thisTextBox.BackColor = Color.Red;
            }
        }
        ...
        private void OKButton_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (!Validation.Validation.EmailValidation(emailTextBox.Text))
            {
        ...
        {
            if (Validation.Validation.TelNumberValidation(telNumberTextBox.Text)
                && Validation.Validation.EmailValidation(emailTextBox.Text)
